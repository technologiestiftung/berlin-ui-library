    ...
    if (options && Array.isArray(options) && options.length > 0) {
        ...
        return (_jsx("div", { className: `flex ${direction === "horizontal" ? "flex-row space-x-4" : "flex-col space-y-2"}`, children: options.map((option) => {
                ...
                return (_jsxs("div", { className: "flex items-center space-x-2", children: [_jsx(Checkbox, { id: `${field.name}-${option.value}`, disabled: isDisabled, checked: isSelected, onCheckedChange: (checked) => {
                            ...
                            } }), _jsx(Label, { htmlFor: `${field.name}-${option.value}`, className: "text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:text-text-disabled", children: option.label || option.value })] }, option.value));
            }) }));
    }
...
function FormFieldWrapper({ formProperty, form, }) {
    ...
    return (_jsx(FormField, { control: form.control, name: id, render: ({ field }) => (_jsxs(FormItem, { children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs(FormLabel, { className: "font-bold", children: [name, isRequired && (_jsx("span", { className: "ml-1 text-decorative-destructive", children: "*" }))] }), helperText && renderTooltip(helperText)] }), _jsx(FormControl, { children: renderField(formProperty, field) }), _jsx(FormDescription, { className: "text-sm text-text-lightest", children: description }), _jsx(FormMessage, {})] })) }, id));
}
