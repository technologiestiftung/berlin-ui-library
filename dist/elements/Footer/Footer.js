...
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger, } from "../../modules/Accordion/Accordion";
...
function FooterContent({ footerColumns = [], berlinFooter, LinkComponent = ({ href, children, ...props }) => (_jsx("a", { href: href, ...props, children: children })), className = "", showScrollToTop = true, }) {
    ...
    const ScrollToTopButton = () => (_jsxs("button", { className: "flex w-48 cursor-pointer gap-x-2 p-2", onClick: () => window.scrollTo({ top: 0, behavior: "smooth" }), children: [translate("toTheTop"), _jsx(ArrowUpIcon, { className: "size-6 bg-red p-1 text-white" })] }));
    return (_jsxs("footer", { className: `w-full bg-block-colored ${className}`, children: [_jsx("div", { className: "flex w-full items-start justify-between lg:p-6", children: _jsxs("div", { className: "flex w-full flex-wrap justify-between lg:items-start lg:pl-[5%]", children: [showScrollToTop && (_jsx("div", { className: "mb-4 flex w-full justify-end lg:hidden", children: _jsx(ScrollToTopButton, {}) })), _jsx("div", { className: "flex w-full flex-col lg:hidden lg:w-auto lg:flex-row lg:flex-wrap", children: _jsx(Accordion, { collapsible: true, type: "single", variant: "footer", children: footerColumns.map((column, index) => (_jsxs(AccordionItem, { value: column.title, children: [_jsx(AccordionTrigger, { children: column.title }), _jsx(AccordionContent, { children: _jsx("div", { className: "flex flex-col", children: _jsx("ul", { className: "flex flex-col gap-6", children: column.links.map((link) => (_jsx("li", { children: _jsxs(LinkComponent, { href: link.href, className: `flex gap-x-1.5 hover:underline ${column.title.includes("socialMedia")
                                                                ? "flex-row"
                                                                : "flex-row-reverse justify-end"}`, children: [translate(link.label), link.icon && link.icon] }) }, link.href))) }) }) })] }, index))) }) }), _jsx("div", { className: "flex w-full flex-col lg:w-auto lg:flex-row lg:flex-wrap", children: footerColumns.map((column) => (_jsxs("div", { className: "mb-6 ml-[3svw] hidden min-w-[12svw] flex-col gap-3.5 lg:flex", children: [_jsx("div", { className: "flex items-center justify-between font-bold", children: _jsx("span", { children: translate(column.title) }) }), _jsx("ul", { className: "flex flex-col gap-2.5 bg-inherit", children: column.links.map((link) => (_jsx("li", { children: _jsxs(LinkComponent, { href: link.href, className: `flex gap-1.5 hover:underline ${column.title.includes("socialMedia")
                                                    ? "flex-row"
                                                    : "flex-row-reverse justify-end"}`, children: [translate(link.label), link.icon && link.icon] }) }, link.href))) })] }, column.title))) }), showScrollToTop && (_jsx("div", { className: "hidden self-start lg:block", children: _jsx(ScrollToTopButton, {}) }))] }) }), berlinFooter && (_jsx("div", { suppressHydrationWarning: true, dangerouslySetInnerHTML: { __html: berlinFooter } }))] }));
}
/**
