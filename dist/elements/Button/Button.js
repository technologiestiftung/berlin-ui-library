...
const renderIconAndBackground = (variant, isDisabled) => {
    if (variant === "light" ||
        variant === "white" ||
        variant === "clean" ||
        variant === "link" ||
    ...
    if (variant === "close") {
        return (_jsx("span", { className: "flex h-full w-full items-center justify-center", children: _jsx(CloseIcon, { className: `size-6 ${isDisabled ? "text-gray-500" : "text-black"}` }) }));
    }
    // Special handling for back variant
    if (variant === "back") {
        return (_jsx("span", { className: "flex h-full w-full items-center justify-center", children: _jsx(ArrowLeftIcon, { className: `size-6 ${isDisabled ? "text-gray-500" : "text-black"}` }) }));
    }
    // Special handling for back-link variant (arrow + text)
    if (variant === "back-link") {
        return (_jsx(ArrowLeftIcon, { className: `mr-1 size-4 ${isDisabled ? "text-gray-500" : "text-black"} flex-shrink-0` }));
    }
    const iconBackgroundColor = isDisabled ? "bg-gray-400" : "bg-red";
    return (_jsxs(_Fragment, { children: [_jsx("span", { className: `absolute top-0 right-0 flex h-full w-[39px] items-center justify-center ${variant.includes("fulltone") ? "" : "border-l-2"} ${isDisabled ? "border-gray-400" : "border-black"} ${iconBackgroundColor}` }), _jsx("span", { className: "pointer-events-none absolute top-0 right-0 flex h-full w-[39px] items-center justify-center", children: variant === "default" ||
                    variant === "colored" ||
                    variant === "fulltone" ||
                    variant === "light-fulltone" ? (_jsx(ArrowRightIcon, { className: "size-5 text-white" })) : variant === "search" ? (_jsx(SearchIcon, { className: "size-[18px] text-white" })) : variant === "filter" ? (_jsx(FilterIcon, { className: "size-4 text-white" })) : variant === "extern" ? (_jsx(LinkIcon, { className: "size-4 text-white" })) : variant === "download" ? (_jsx(DownloadIcon, { className: "size-[18px] text-white" })) : variant === "addtocart" ? (_jsx(CartIcon, { className: "size-[18px] text-white" })) : variant === "play" ? (_jsx(PlayIcon, { className: "size-[18px] text-white" })) : null })] }));
};
...
function Button({ className, variant = "default", size, asChild = false, booking = false, price, children, disabled = false, ...props }) {
    const Comp = asChild ? Slot : "button";
    const bookingContent = booking && price ? (_jsxs(_Fragment, { children: [_jsx("span", { className: "price mr-[0.7em] border-r border-black/60 pr-[0.7em]", children: price }), _jsx("span", { children: children })] })) : (children);
    return (_jsxs(Comp, { "data-slot": "button", className: cn(buttonVariants({ variant, size, className }), booking ? "button--booking flex" : ""), disabled: disabled, ...props, children: [bookingContent, variant === "linkWithIcon" && (_jsx(LinkIcon, { className: `mb-2 ml-1 inline-block size-[16px] align-text-top leading-none ${disabled ? "text-gray-500" : "text-[#0047d3]"}`, "aria-hidden": "true" })), renderIconAndBackground(variant, disabled)] }));
}
