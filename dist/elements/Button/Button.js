...
const renderIconAndBackground = (variant, isDisabled) => {
    ...
    if (variant === "close") {
        return (_jsx("span", { className: "flex h-full w-full items-center justify-center", children: _jsx(CloseIcon, { className: `${isDisabled ? "text-gray-500" : "text-black"}` }) }));
    }
    // Special handling for back variant
    if (variant === "back") {
        return (_jsx("span", { className: "flex h-full w-full items-center justify-center", children: _jsx(ArrowLeftIcon, { className: `${isDisabled ? "text-gray-500" : "text-black"}`, width: "24", height: "24" }) }));
    }
    // Special handling for back-link variant (arrow + text)
    if (variant === "back-link") {
        return (_jsx(ArrowLeftIcon, { className: `mr-1 ${isDisabled ? "text-gray-500" : "text-[#0047d3]"}`, width: "16", height: "16" }));
    }
    ...
    return (_jsxs(_Fragment, { children: [_jsx("span", { className: `absolute top-0 right-0 flex h-full w-[39px] items-center justify-center ${variant.includes("fulltone") ? "" : "border-l-2"} ${isDisabled ? "border-gray-400" : "border-black"} ${iconBackgroundColor}` }), _jsx("span", { className: "pointer-events-none absolute top-0 right-0 flex h-full w-[39px] items-center justify-center", children: variant === "default" ||
                    variant === "colored" ||
                    variant === "fulltone" ||
                    variant === "light-fulltone" ? (_jsx(ArrowRightIcon, { className: "text-white" })) : variant === "search" ? (_jsx(SearchIcon, { className: "size-[18px] fill-white" })) : variant === "filter" ? (_jsx(FilterIcon, { className: "fill-white" })) : variant === "extern" ? (_jsx(LinkIcon, { className: "text-white" })) : variant === "download" ? (_jsx(DownloadIcon, { className: "size-[18px] fill-white" })) : variant === "addtocart" ? (_jsx(CartIcon, { className: "size-[18px] fill-white" })) : variant === "play" ? (_jsx(PlayIcon, { className: "size-[18px] fill-white" })) : null })] }));
};
