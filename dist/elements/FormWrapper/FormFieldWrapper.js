...
const renderRadioGroup = (property, field) => {
    ...
    return (_jsx(RadioGroup, { value: field.value, onValueChange: field.onChange, disabled: isDisabled, className: `flex ${direction === "horizontal" ? "flex-row space-x-4" : "flex-col space-y-1"}`, children: options?.map((option) => (_jsxs("div", { className: "flex items-center space-x-2", children: [_jsx(RadioGroupItem, { id: `${field.name}-${option.value}`, value: option.value }), _jsx(Label, { htmlFor: `${field.name}-${option.value}`, className: "leading-[1.2] peer-disabled:cursor-not-allowed peer-disabled:text-text-disabled", children: option.label || option.value })] }, option.value))) }));
};
...
function FormFieldWrapper({ formProperty, form, }) {
    ...
    return (_jsx(FormField, { control: form.control, name: id, render: ({ field }) => (_jsxs(FormItem, { children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs(FormLabel, { className: "font-bold", children: [name, isRequired && (_jsx("span", { className: "ml-1 text-[1.75em] text-decorative-destructive align-top leading-none", children: "*" }))] }), helperText && renderTooltip(helperText)] }), _jsx(FormControl, { children: renderField(formProperty, field) }), _jsx(FormDescription, { className: "text-sm text-text-lightest", children: description }), _jsx(FormMessage, {})] })) }, id));
}
